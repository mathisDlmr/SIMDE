...
- _Laravel c'est facile à comprendre, ça permet une formation rapide pour reprendre en main les services proposés_
...
Les fichiers sont ensuite récupérés par un Apache qui sert les fichiers statiques et les applications web.
...
![Formulaire de connexion](filezilla-2.png)
...
Cela peut être dû à de mauvaises config de .htaccess ou de mauvaise version d'interpréteur sur les machines.
...
on:     # Évènements qui déclenchent l'action
  push:
    branches: [ main ]
  pull_request:
    branches: [ "main" ]
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Prepare deploy folder   # Prépare le dossier de deploy
        run: |
          mkdir deploy
      ...
      - name: Upload artifact    #Créé l'artifact de déploiement
...
_De manière optionnelle, il est possible de configurer la couleur et l'image qui seront affichées lors de la demande d'autorisation d'accès aux informations._
